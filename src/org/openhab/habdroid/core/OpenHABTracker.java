...
//                new IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION));
        ...
        if (activeNetworkInfo != null) {
            ...
            if (settings.getBoolean(Constants.PREFERENCE_DEMOMODE, false)) {
                ...
                if (activeNetworkInfo.getType() == ConnectivityManager.TYPE_MOBILE) {
                    ...
                    System.out.println("\n OpenHABTracker->8->openHabURL->"+mOpenHABUrl);
                ...
                // Else if we are on Wifi or Ethernet network
                    ...
                    System.out.println("\n OpenHABTracker->11->openHABURL->"+mOpenHABUrl);
                    ...
                    if (mOpenHABUrl.length() > 0) {
                    	...
                    	if (true) {
                        	System.out.println("\n OpenHABTracker->13->openHABURL->"+mOpenHABUrl);
                            ...
                            if (mOpenHABUrl.length() > 0) {
                                Log.d(TAG, "Connecting to remote URL " + mOpenHABUrl);
                                System.out.println("\n OpenHABTracker->15->openHABURL->"+mOpenHABUrl);
                                openHABTracked(mOpenHABUrl, mCtx.getString(R.string.info_conn_rem_url));
                            } else {
                            	System.out.println("\n OpenHABTracker->16->Error");
                                openHABError(mCtx.getString(R.string.error_no_url));
                            }
                        }
                    // If no local URL is configured
                    } else {
                    	System.out.println("\n OpenHABTracker->17->Duscovery");
    ...
    private void openHABTracked(String openHABUrl, String message) {
        if (mReceiver != null){
            System.out.println("\nopneHABTracker->receiver not null->"+mReceiver);
        	mReceiver.onOpenHABTracked(mOpenHABUrl, message);
        	System.out.println("\nopneHABTracker->receiver executed");
        }
    }
